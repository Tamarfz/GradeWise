...
const Analytics = observer(() => {
  const [viewMode, setViewMode] = useState('judge');
    ...
    try {
      ...
      gradesData.grades.forEach(grade => {
        ...
        if (!existingProject) {
          ...
          // Check if project is graded (not null values)
          const isGraded = !(grade.complexity === null && grade.usability === null && 
                           grade.innovation === null && grade.presentation === null && 
                           grade.proficiency === null);
  ...
  const calculateAverageGrades = () => {
    ...
    if (viewMode === 'judge' && analyticsData.projects) {
      // For judge view: include only projects with real grades (exclude nulls)
      validGrades = analyticsData.projects.filter(project => (
        project.avgComplexity !== null &&
        project.avgUsability !== null &&
        project.avgInnovation !== null &&
        project.avgPresentation !== null &&
        project.avgProficiency !== null &&
        project.avgTotal !== null
      ));
    } else if (viewMode === 'project' && analyticsData.judges) {
      // For project view: include only judges with real grades (exclude nulls)
      validGrades = analyticsData.judges.filter(judge => (
        judge.complexity !== null &&
        judge.usability !== null &&
        judge.innovation !== null &&
        judge.presentation !== null &&
        judge.proficiency !== null &&
        judge.totalGrade !== null
      ));
    }
  ...
  const isNotGraded = (judge) => {
    // Check if all scores are null (not graded)
    return judge.complexity === null && judge.usability === null && 
           judge.innovation === null && judge.presentation === null && 
           judge.proficiency === null;
  };

  const isProjectGraded = (project) => {
    // Check if any of the average scores are not null (meaning it has been graded)
    return !(project.avgComplexity === null && project.avgUsability === null && 
             project.avgInnovation === null && project.avgPresentation === null && 
             project.avgProficiency === null);
  };
    ...
    if (viewMode === 'judge') {
      return (
        <div className="analytics-table-container">
          ...
          <table className="analytics-table view-by-judge">
            <thead>
              <tr>
                <th>Project</th>
                <th>Complexity</th>
            ...
            <tbody>
                ...
                analyticsData.projects.map((project, index) => (
                  <tr key={index}>
                    <td>{project.title}</td>
                    <td>{formatGradeValue(project.avgComplexity)}</td>
                    <td>{formatGradeValue(project.avgUsability)}</td>
                    <td>{formatGradeValue(project.avgInnovation)}</td>
                    <td>{formatGradeValue(project.avgPresentation)}</td>
                    <td>{formatGradeValue(project.avgProficiency)}</td>
                    <td>{formatGradeValue(project.avgTotal)}</td>
                    <td>
      ...
      return (
        <div className="analytics-table-container">
          ...
          <table className="analytics-table view-by-project">
            <thead>
              <tr>
                <th>Judge</th>
                <th>Complexity</th>
            ...
            <tbody>
              {analyticsData.judges && analyticsData.judges.length > 0 ? (
                ...
                <tr key={index}>
                  <td>{judge.name}</td>
                  <td>{formatGradeValue(judge.complexity)}</td>
                  <td>{formatGradeValue(judge.usability)}</td>
                  <td>{formatGradeValue(judge.innovation)}</td>
                  <td>{formatGradeValue(judge.presentation)}</td>
                  <td>{formatGradeValue(judge.proficiency)}</td>
                  <td>{formatGradeValue(judge.totalGrade)}</td>
                  <td>
                    {isNotGraded(judge) ? (
                      <span className="not-graded-badge">Not Graded</span>
                    ) : (
  ...
  const renderAdditionalInfoBox = () => {
    ...
    // Filter out judges with null scores (not graded)
  ...
  const renderJudgeAssessmentSummary = () => {
    ...
    // Filter out projects with null scores (not graded)
