...
.then((collections) => {
  router.get('/current-judge', async (req, res) => {
    try {
      ...
      // Ensure that the user is a judge
      if (userFromToken.type !== 'judge') {
        return res.status(403).json({ error: 'Forbidden: Current user is not a judge.' });
      }

      // Debug: log the token user object to check its properties
      console.log('userFromToken:', userFromToken);

      // Try using both properties in case of mismatch:
      const judgeData = await collections.users.findOne({
        $or: [{ ID: userFromToken.ID }, { ID: userFromToken.id }]
      });
